...
@timeit
def bestFirstSearch(graph, start, target, energy_weight=1, distance_weight=1, max_energy_cost=287932):
    logger = progress_display(log_every=10000)

    graph = {k:v for k,v in graph.items()}

    avg_distance = sum(distancedict.values()) / len(distancedict)
    avg_energy   = sum(costdict.values()) / len(distancedict)
    ...
    while queue:
        ...
        for node in neighbours:
            ...
            edge_distance, edge_energy = distancedict[edge_key], costdict[edge_key]
...
if __name__ == "__main__":
    ...
    dist, energy, path = bestFirstSearch(graphdict, '1', '50', energy_weight=11, distance_weight=1)
    path = "->".join(path)
    print()
    print(f"{dist=}\n{energy=}\n{path=}")
    print('\n')
    print('Params optimised for energy\n', '-'*50)
    dist, energy, path = bestFirstSearch(graphdict, '1', '50', energy_weight=1, distance_weight=6)
    path = "->".join(path)
    print()
    print(f"{dist=}\n{energy=}\n{path=}")
